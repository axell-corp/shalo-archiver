...
バージョン 1.0.0 時点で、本ソフトウェアには以下のような制約があります。

- SHALO AUTH 内に入っている ECDSA 鍵を用いて暗号化できない。
- SHALO AUTH に ECDSA 鍵が含まれる場合、復号用の RSA 鍵があったとしても復号に失敗する。
- 鍵のインポート時に、RSA2048bit 以外の形式の鍵を使用できない。

以上の事項は、今後のアップデートにより対応・修正する予定です。
